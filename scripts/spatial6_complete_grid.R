...
# load grid
grid_sf <- sf::st_read(paste0("data_outputs/3_cell_grid/5_grid_lulc/",set_name,"_grid_lulc_",param_plot_km,"km.shp"))
# load raster
soy_rast <- terra::rast("data_outputs/3_cell_grid/1_reproject/fao_brazil_soy_yield.tif")
# check coordinate reference systems
if(sf::st_crs(grid_sf) != sf::st_crs(soy_rast)) {
  warning("CRS mismath. Reprojecting grid to raster CRS for extraction.")
  soy_rast <- terra::project(soy_rast, terra::crs(vect(grid_sf)), method="bilinear")
}

# (2) process raster ------------------------------------------------------
# exact_extract returns a data.frame with 'cell_id' and columns for each class
plot_yield <- exactextractr::exact_extract(soy_rast, grid_sf,
                                           weights="area",
                                           fun="weighted_mean",
                                           coverage_area=TRUE,
                                           force_df=TRUE,
...
grid_df <- grid_sf %>% sf::st_drop_geometry()
grid_sf <- grid_sf %>% dplyr::select(cell_id, geometry)
