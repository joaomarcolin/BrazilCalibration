# 2026 05 09
# This script calls all other scripts that deal with data from IBGE, Mapbiomas, 
# IPEA, and Paraná's Secretary of Agriculture in order to generate:
# (1) 'sf_grid': shapefile dividing Brazil into cells of a given size
# (2) 'df_grid': dataframe associating each cell in 'sf_grid' with values of spatial variables in 1985, 1995, 2006, 2017 and 2024
# (3) 'df_brazil_mun': data.frame with all municipality identifiers and grouping variables, municipality area (protected or not) in 2024, and average soybean yield in 1981-2010 according to FAO GAEZ
# (4) 'df_census_mun': data.frame with municipality-level data from the agricultural censuses of 1985, 1995, 2006, and 2017
# (5) 'df_census_state': data.frame with state-level data from the agricultural censuses of 1985, 1995, 2006, and 2017
# (6) 'df_yearly_mun': data.frame with municipality-level yearly data on LULC, herd and soybean output for 1985-2024
# (7) 'df_yearly_prices': data.frame with yearly data on base interest rates and minimum wage for 1985-2024, as well as price of beef and soybean in 1995-2024
...
# determine size of cells in the cell grid
param_plot_km <- 5
...
source("scripts/spatial1_reproject.R")     # reproject shapefiles and rasters (takes a very long time)
source("scripts/spatial2_recode_lulc.R")   # reclass Mapbiomas' rasters       (takes a very long time)
...
readr::write_csv(df_grid, paste0("data_outputs/1_cell_grid/6_complete_grid/df_brazil_grid_",param_plot_km,"km.csv"))
sf::st_write(sf_grid,     paste0("data_outputs/1_cell_grid/6_complete_grid/sf_brazil_grid_",param_plot_km,"km.shp"), delete_layer=TRUE)
